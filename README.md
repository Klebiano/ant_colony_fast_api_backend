...
This backend serves as the reference implementation and computational engine for the companion [Wind Turbines Route Optimization Dashboard](https://github.com/Klebiano/wind_turbines_route_optimization_dashboard) frontend application.

The entire backend architecture and metaheuristic optimization algorithms were also replicated in **Golang** and **Rust** to benchmark execution speed, memory footprint, and HTTP API latency against this Python FastAPI version:
👉 **[ant_colony_golang_rust_backend](https://github.com/Klebiano/ant_colony_golang_rust_backend)**

---

## 🖥️ Frontend Dashboard

The user interface for visualizing wind farm layouts, configuring turbine fault scenarios, and viewing computed optimization routes is available at:
👉 **[wind_turbines_route_optimization_dashboard](https://github.com/Klebiano/wind_turbines_route_optimization_dashboard)**

* **Interactive Turbine Map**: Visual offshore map displaying wind turbines and dock locations.
* **Failure Simulation**: Select turbine subsystems and fault severity to calculate downtime.
* **Live Route Animation & Metrics**: Real-time rendering of optimal navigation cycles (ACO, GA, Memetic) generated by this FastAPI service.

---

## 🏎️ Golang & Rust Replications & Performance Comparisons

To explore high-performance systems programming paradigms and compare execution metrics against Python FastAPI, the complete backend—including Ant Colony Optimization, Genetic Algorithms, Memetic Algorithms (2-opt), and REST APIs—was re-engineered in **Go** and **Rust**:

* **Repository**: [ant_colony_golang_rust_backend](https://github.com/Klebiano/ant_colony_golang_rust_backend)
* **Go Implementation**: Built with standard library `net/http`, custom CORS middleware, and `database/sql` + SQLite driver.
* **Rust Implementation**: Built with `axum`, `tokio` async runtime, `tower-http`, and `rusqlite`.
* **Benchmark Insights**: The compiled Go and Rust implementations achieve up to **850x+ faster algorithmic execution speeds** and **40x+ reduced HTTP API response latency** compared to the Python baseline while maintaining exact parity in routing outputs.
...
- ⚡ **FastAPI REST API**: Interactive OpenAPI / Swagger interface for easy integration with frontend dashboards (e.g., [wind_turbines_route_optimization_dashboard](https://github.com/Klebiano/wind_turbines_route_optimization_dashboard), React, Vite, Next.js).
